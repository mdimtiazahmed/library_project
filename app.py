...
def add_member():
    ...
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO members (name, email, phone, address) VALUES (%s, %s, %s, %s)",
...
def issue_book():
    ...
    cur.execute("SELECT member_id, name FROM members WHERE status='active'")
    active_members = cur.fetchall()
    cur.close()
    return render_template('issue_book.html', books=available_books, members=active_members)
...
def return_book(id):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    cur = mysql.connection.cursor()
    cur.execute("SELECT book_id FROM transactions WHERE transaction_id = %s", (id,))
    transaction = cur.fetchone()
    cur.execute("UPDATE transactions SET return_date = CURDATE() WHERE transaction_id = %s", (id,))
    cur.execute("UPDATE books SET available_copies = available_copies + 1 WHERE book_id = %s", (transaction[0],))
    mysql.connection.commit()
    cur.close()
